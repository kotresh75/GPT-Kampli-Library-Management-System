            ...
            <div className="glass-panel bounce-in" style={{ width: '700px', maxHeight: '80vh', display: 'flex', flexDirection: 'column', padding: 0, background: 'var(--bg-color)', border: '1px solid var(--glass-border)' }}>
                ...
                <div style={{ padding: '20px', borderBottom: '1px solid var(--glass-border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'rgba(255,255,255,0.05)' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                        ...
                        <div>
                            <h2 style={{ fontSize: '1.1rem', fontWeight: 'bold', margin: 0 }}>Activity Logs</h2>
                            <span style={{ fontSize: '0.8rem', color: 'var(--text-secondary)' }}>For {admin.name}</span>
                        </div>
                    </div>
                ...
                <div style={{ padding: '0', overflowY: 'auto', flex: 1 }}>
                    {loading ? (
                        ...
                        <div style={{ padding: '40px', textAlign: 'center', color: 'var(--text-secondary)' }}>No activity recorded yet.</div>
                    ) : (
                        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.9rem' }}>
                            <thead style={{ background: 'rgba(255,255,255,0.02)', position: 'sticky', top: 0 }}>
                                <tr>
                                    <th style={{ padding: '12px 20px', textAlign: 'left', fontWeight: 600, color: 'var(--text-secondary)' }}>Type</th>
                                    <th style={{ padding: '12px 20px', textAlign: 'left', fontWeight: 600, color: 'var(--text-secondary)' }}>Module</th>
                                    <th style={{ padding: '12px 20px', textAlign: 'left', fontWeight: 600, color: 'var(--text-secondary)' }}>Description</th>
                                    <th style={{ padding: '12px 20px', textAlign: 'right', fontWeight: 600, color: 'var(--text-secondary)' }}>Time</th>
                                </tr>
                            </thead>
                            <tbody>
                                {logs.map(log => (
